...
<script>
  ...
  import {
    reportStadisticDiabeticPatientComorbidApi,
  } from '@/api/modules'
  export default {
    data () {
      return {
    ...
    methods: {
      ...
      async listItem () {
        const serviceResponse = await reportStadisticDiabeticPatientComorbidApi(this.filter)
      ...
      async listItemPathology () {
        ...
        if (serviceResponse.ok) {
          this.pathologys = serviceResponse.data.filter(item => item.id !== 1)
        } else {
          this.alert({
            text: serviceResponse.message.text,
            color: 'warning',
