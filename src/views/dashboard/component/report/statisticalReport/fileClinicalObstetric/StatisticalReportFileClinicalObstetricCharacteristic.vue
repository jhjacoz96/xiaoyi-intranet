<template>
  <v-container>
    ...
    >
      ...
      >
        <v-select
          v-model="filter.pathologyPregnant"
          label="Patologias prenatales"
          outlined
          dense
          multiple
          item-text="nombre"
...
<script>
  ...
  import {
    reportStadisticFileObstatricCharacteristicApi,
    gestationWeekAllApi,
  ...
  export default {
    ...
    methods: {
      ...
      async listItem () {
        const serviceResponse = await reportStadisticFileObstatricCharacteristicApi(this.filter)
      ...
      async listItemGestation () {
        ...
        if (serviceResponse.ok) {
          this.gestation = serviceResponse.data
        } else {
          this.alert({
            text: serviceResponse.message.text,
            color: 'warning',
