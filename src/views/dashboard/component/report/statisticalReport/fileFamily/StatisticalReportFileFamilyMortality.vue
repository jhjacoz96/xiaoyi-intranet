<template>
  <v-container>
    ...
    <v-card-text class="text-center text-h5 font-weight-medium">
      Cantidad de defunción incluidad en el reporte: <span v-if="data">{{ data.total }}</span><span v-else>0</span>
    </v-card-text>
    <v-card-text>
...
<script>
  ...
  import {
    reportStadisticFilefamilyMortalityApi,
  } from '@/api/modules'
  export default {
    data () {
      return {
    ...
    methods: {
      ...
      async listItem () {
        const serviceResponse = await reportStadisticFilefamilyMortalityApi(this.filter)
