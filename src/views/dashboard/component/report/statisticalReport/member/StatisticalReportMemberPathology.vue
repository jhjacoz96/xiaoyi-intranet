<template>
  <v-container>
    ...
    <v-card-text class="text-center text-h5 font-weight-medium">
      Cantidad de miembros con patologías  incluidados en el reporte: <span v-if="data">{{ data.total }}</span><span v-else>0</span>
    </v-card-text>
    <v-card-text>
...
<script>
  ...
  import {
    reportStadisticMemberPathologyApi,
  } from '@/api/modules'
  export default {
    data () {
      return {
    ...
    methods: {
      ...
      async listItem () {
        const serviceResponse = await reportStadisticMemberPathologyApi(this.filter)
